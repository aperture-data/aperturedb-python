...
def test_Images_search():
    ...
    with patch('aperturedb.Images.execute_query') as mock_execute:
        mock_execute.return_value = (0, [{"FindImage": {"entities": [{"_uniqueid": "123"}, {"_uniqueid": "456"}]}}], [])
        img = Images(client)
        img.search(limit=2)
        assert "123" in img.images_ids
        assert "456" in img.images_ids
        mock_execute.assert_called_once()
        query_passed = mock_execute.call_args[1]["query"] if "query" in mock_execute.call_args[1] else mock_execute.call_args[0][1]
...
def test_Images_search_by_property():
    client = MockClient()
    with patch('aperturedb.Images.execute_query') as mock_execute:
        mock_execute.return_value = (0, [{"FindImage": {"entities": [{"_uniqueid": "789"}]}}], [])
        img = Images(client)
