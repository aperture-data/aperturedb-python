...
# from aperturedb import Entities, Utils
...
# class Images(Entities.Entities):
#     db_object = "Image"
...
#     def __retrieve_keypoints(self, index):
#         query = [{
#             "FindImage": {
#                 "_ref": 1,
#                 "constraints": {
...
#         self.images_keypoints = self.get_properties(prop_list=["keypoints"])
...
#         query["FindImage"]["results"]["list"].append("keypoints")
...
#                 kpl.extend([int(points[i]), int(points[i + 1]), 2])
#                 # kpl.append( {
#                 #     "x": int(points[i]),
#                 #     "y": int(points[i + 1]),
#                 #     "v": 2} )
#                 # kpl.append((int(points[i]), int(points[i+1]), 2))
#             # for i in points:
#                 # kpl.append(int(i))
#             return kpl
...
#                 # self.images_keypoints.append(get_kps(ent["keypoints"].split()))
...
#         if not limit:
#             limit = min(self.display_limit, len(self.images_ids))
#             if self.display_limit < len(self.images_ids):
#                 print("Showing only first", limit, "results.")
#         self.cur_width = 200

#         def render_images(show_bboxes, show_segmentation, show_keypoints):
#             images = []
...
#                 # image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
#                 # if show_bboxes:
#                 #     image = cv2.rectangle(image, (0, 0), (50, 50), (0, 0, 255), 2)
#                 if show_keypoints:
#                     for kp in self.images_keypoints[i]:
#                         x, y = kp
#                         image = cv2.circle(
#                             image, (x, y), radius=4, color=(0, 255, 0), thickness=2)
#                 image = cv2.imencode('.jpg', image)[1].tobytes()
#                 image = widgets.Image(
#                     value=image, format="JPEG", width=self.cur_width)
#                 images.append(image)

#             return images

#         self.r_images = render_images(False, False, False)
#         box2 = widgets.Box(children=self.r_images, layout={
#             'display': 'inline'
#         })

#         # Controls for interaction
#         def test1(b):
#             self.cur_width = int(b['new'])
#             for im in self.r_images:
#                 im.width = self.cur_width

#         op = widgets.Output()
#         options = ["Bounding boxes", "Segmentations", "Key points"]
#         annotations = {
#             op: False for op in options
#         }
...
#                 annotations[c['owner'].description] = c['new']
#                 print(annotations)
#                 self.r_images = render_images(
#                     show_bboxes=annotations["Bounding boxes"],
#                     show_segmentation=annotations["Segmentations"],
#                     show_keypoints=annotations["Key points"]
#                 )
#                 box2 = widgets.Box(children=self.r_images, layout={
#                     'display': 'inline'
#                 })
...
#         op_widgets = [widgets.Checkbox(
#             value=False, description=option) for option in options]
#         for w in op_widgets:
#             w.observe(test2, 'value')

#         sizer = widgets.IntSlider(min=1, max=400, value=200)
#         sizer.observe(test1, 'value')
#         op_widgets.append(sizer)

#         box1 = widgets.Box(children=op_widgets, layout=layout1)
#         box = widgets.VBox(children=[box1])
#         # with op:
#         display(box, op)
#         with op:
#             display(box2)
