...
def _create_configuration_from_json(config: Union[Dict, str],
    ...
    **Create a connector to the database from a JSON configuration.**
    ...
    assert "host" in config, f"host is required in the configuration: {
        clean_config}"
    assert "username" in config, f"username is required in the configuration: {
        clean_config}"
    assert "password" in config, f"password is required in the configuration: {
        clean_config}"
    ...
    if name_required:
        assert "name" in config, f"name is required in the configuration: {
            clean_config}"
